//! Windows `CF_HDROP` clipboard access — file paths to and from the shell
//! drop list.
//!
//! Shared by both ends of WireDesk: the host reads Explorer copies and
//! injects incoming files, and the Windows client does exactly the same on
//! its own machine. It lived in `wiredesk-host` until the client gained a
//! Windows build; the code moved here unchanged rather than being copied.
//! The macOS counterpart is `NSPasteboard`, which each side wraps locally
//! (`apps/wiredesk-client/src/clipboard_files.rs`).
//!
//! ## Scope
//!
//! - **Single-file only** for Phase 1. Multi-file selections (the HDROP
//!   handle reports `count != 1` via `DragQueryFileW(0xFFFFFFFF)`) are
//!   silently skipped with a debug log. Multi-file is a separate brief
...
//! manager on success — the caller must NOT free. On the failure path the
//! caller still owns the handle and must call `GlobalFree`. `set_cf_hdrop`
//! handles both paths correctly.
//!
//! ## non-Windows targets
//!
//! On non-Windows this module compiles to no-op stubs returning `None` /
//! `Err(FileClipboardError::ClipboardLocked)` so call sites don't need
//! `#[cfg]` guards.
