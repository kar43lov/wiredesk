//! Windows BLE Peripheral implementation. Substantive WinRT GATT-server
//! wiring lands in Task 7 of `docs/plans/20260506-bluetooth-le-transport.md`.
//! For now this module exposes the same struct + config shape as the stub so
//! the crate compiles on Windows while the real implementation is in flight.
...
impl BluetoothLeTransport {
    pub fn open(_cfg: &BluetoothFactoryConfig) -> Result<Self> {
        Err(WireDeskError::Transport(
            "BLE Peripheral impl pending (Task 7)".to_string(),
        ))
    }
}
...
impl Transport for BluetoothLeTransport {
    fn send(&mut self, _packet: &Packet) -> Result<()> {
        unimplemented!("BLE send pending Task 7")
    }

    fn recv(&mut self) -> Result<Packet> {
        unimplemented!("BLE recv pending Task 7")
    }
    ...
    fn try_clone(&self) -> Result<Box<dyn Transport>> {
        Err(WireDeskError::Transport(
            "BLE try_clone pending Task 7".to_string(),
        ))
    }
}
...
mod tests {
    ...
    fn name_is_stable() {
        ...
        assert_eq!(t.name(), "bluetooth-le-peripheral");
    }
}
