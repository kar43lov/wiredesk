...
«не хуже 10 %» им не выполнен, и это надо либо принять явно, либо делать follow-up с двумя
очередями приоритета (см. строку 240).

**Не сделано:** PR не открыт (`gh` блокируется классификатором), CI на ветке не гонялся —
он идёт только на push в `main` и на PR, поэтому **Windows-раннер этот код не собирал**.
