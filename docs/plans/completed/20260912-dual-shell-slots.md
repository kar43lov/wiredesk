...
>
> 🔴 **Долг:** AC2 по букве не выполнен (критерий требовал ≤10 %, факт ×4 — заложенный
> размен бюджета канала); AC3 / AC6 / AC8 не проверялись.
...
🔴 **По AC2 решение за владельцем.** Замедление ×4 — не дефект, а заложенный в план размен
бюджета канала (`PUMP_BUDGET_EXEC_SHARED = 4` против `PUMP_BUDGET = 16`), и до этой правки
та же пара команд не работала вовсе (`shell busy`, exit 125). Но записанный здесь критерий
«не хуже 10 %» им не выполнен, и это надо либо принять явно, либо делать follow-up.

Follow-up — на хосте, а не в клиенте: две очереди приоритета у клиентского writer'а (раздел
«Что сознательно НЕ делается») ускорили бы нажатия, а медленный здесь обратный поток —
вывод exec. Урезанный бюджет включается по `self.pty.is_some()` (`session.rs:453`), то есть
и тогда, когда консоль открыта, но молчит. Слот PTY качается первым, поэтому достаточно
урезать exec, только если PTY в этом же такте что-то отдал.
