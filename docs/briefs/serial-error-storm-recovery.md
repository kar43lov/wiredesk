...
> **SHIPPED & VERIFIED LIVE 2026-06-04, master `6f35c34`** (fast-forward от `feature/error-storm-recovery`, план `docs/plans/completed/20260604-error-storm-recovery.md`). StormCounter (10 подряд Protocol-ошибок) на обеих сторонах + Win reopen-loop + Mac LinkSupervisor с backoff 1s→30s + IPC `TransportUnavailable`/exit 125 + UI Reconnecting. Codex-ревью: 6 итераций, 7 фиксов (включая drain-гонку, handshake-gate, release_all при reopen). 658 тестов. Live-verified пользователем на Mac M4 + Win11 + FT232H.

**Status:** SHIPPED. Исторический бриф ниже.
