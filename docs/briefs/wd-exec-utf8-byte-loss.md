...
**Status: ИСПРАВЛЕНО 2026-09-11.** Причина оказалась не той, что здесь
предполагалась, и всё, что ниже про flow-control, chunked-handshake и
logic-analyzer, делать не нужно — байты никогда не терялись.

Host режет вывод shell'а на пакеты по `MAX_PAYLOAD`, граница пакета падает
куда придётся, а `run_oneshot` декодировал каждый пакет отдельно через
`String::from_utf8_lossy`. Кириллическая буква, попавшая на шов, разрезается
пополам: хвост первого пакета — «обрывок последовательности», голова второго —
«продолжение без начала», и каждая половина превращается в `U+FFFD`. Отсюда и
ровно тот симптом, что описан ниже: **два** replacement-символа на месте
**одной** буквы, примерно раз на 4 КБ, и только в неASCII-тексте. `--compress`
проблему не решал, а прятал: base64 — это ASCII, резать его по любому месту
безопасно.

Лечение — `crates/wiredesk-exec-core/src/utf8_stream.rs`: декодер держит
незавершённый символ (не больше трёх байт) до следующего пакета. Байт, который
действительно некорректен, по-прежнему становится `U+FFFD`, так что бинарь на
stdout деградирует как раньше. Регрессия закрыта двумя тестами в
`runner.rs` — буква, разрезанная пополам, и весь вывод по одному байту;
на старом коде первый из них даёт `о��ибок` вместо `ошибок`.

🔴 **Что здесь стоит запомнить про сам бриф.** Все три гипотезы ниже
(CP1251, переполнение буфера CH340, гонка sentinel'а) указывали на железо и
на физический уровень, а дефект был в одной строке разбора и
воспроизводился бы на mock-транспорте за минуту. Признак, который на это
указывал, в тексте уже лежал: `��` — это ДВА replacement-символа, то есть
не «потерянные байты», а две половины одного символа, декодированные
порознь. Потеря на UART так не выглядит.

---

**Исходный бриф (гипотезы не подтвердились, оставлен как есть):**
