...
1. **`wd --exec` и `WireDesk.app` теперь работают параллельно** (Mac, начиная с feat/wd-exec-ipc). GUI на старте поднимает Unix-socket в `~/Library/Application Support/WireDesk/wd-exec.sock`, `wd --exec` коннектится к нему и ходит через тот же serial, который GUI использует для clipboard sync. Если GUI закрыт — `wd --exec` falls back на direct-open serial (поведение идентично pre-implementation). **Interactive `wd`** (без `--exec`, PTY-mode bridge для Ghostty/iTerm) с 2026-07-03 ходит через тот же сокет — GUI закрывать не нужно. С 2026-09-12 они ещё и не мешают друг другу: у хоста два shell-слота, и `wd --exec` выполняется при открытой консоли. `shell busy` (exit 125) теперь значит одно из двух — либо хост не пересобран и живёт на одном слоте (в `client.log` строка `connected to '<host>' (… proto v1)`), либо это второй **интерактивный** `wd`, а pty-слот один в любом случае.
...
| 125 | Transport error (serial drop'нулся, host исчез) **или** `shell busy` — на старом хосте открыт интерактивный `wd` (см. п. 1 выше; на пересобранном хосте этого отказа больше нет) **или** GUI serial-link mid-reconnect (IPC mode: GUI чинит канал после frame-error storm / disconnect — на stderr `wd: transport reconnecting — retry shortly`, retry через пару секунд) **или** `--compress` decode failure (host выдал невалидный gzip+base64 payload — на stderr печатается `--compress decode failed: <msg>`) |
...
- **Длинная команда разбирается на хосте, а не едет по каналу.** У PowerShell разбор одной строки растёт быстрее длины: замер 2026-09-11 на Win11 — 4 КБ за 46 мс, 48 КБ уже за 14.8 с, 96 КБ не успевали в 90-секундный таймаут. При этом сами байты доезжали за 0.3 с, а запись в stdin стоила микросекунды — транспорт был ни при чём. Хост теперь кладёт payload длиннее 4 КБ во временный `.ps1` в `%TEMP%` (UTF-8 с BOM) и скармливает shell'у одну строку `. ([scriptblock]::Create([IO.File]::ReadAllText('файл')))` — dot-source исполняется в той же области, так что `$LASTEXITCODE`, переменные и sentinel ведут себя как раньше, а script block из строки не спотыкается об `ExecutionPolicy`. Кладётся **весь** payload целиком, включая многострочный: переносить в файл только первую строку нельзя, она бывает незакрытой половиной `try { … }`. Стало: 48 КБ одной строкой — 0.49 с, 39 КБ в 3000 строк — 0.47 с (до этого такой скрипт не выполнялся вовсе, см. ниже). Файл удаляется той же строкой сразу после исполнения, но в этот момент текст команды всё же лежит на диске — секреты лучше передавать через переменные окружения хоста, а не в теле команды.

### SSH path
- **Многострочная команда до 2026-09-11 не выполнялась вообще.** Консольный хост PowerShell читает stdin как клавиатуру: незакрытый конструкт включает continuation-режим, и выйти из него можно только **пустой строкой** — закрывающей скобки мало. Поэтому многострочный payload оставался в буфере, `wd --exec` возвращал `rc=0` и пустой вывод, а команда не запускалась. Обёртка теперь дописывает пустую строку, если в payload есть перевод строки. Проверено живьём: `if`-блок, here-string и 3000-строчный скрипт.
...
### Sequential calls
- Два `wd --exec` подряд работают (host slot free'ится между ними через ShellClose+Disconnect). Но **между ними ~3.5-4 сек overhead каждый раз** (handshake ~2s + drain ~2s). Для серии команд лучше — одна команда с `;` или `&&`.
- **После heavy-failed cmd'а** (output >100KB → timeout 124) drain может занять до 30s (cap), пока host не докончит шипить tail. Это намеренно — иначе next cmd сел бы на saturated wire и тоже бы упал. Drain-marker в `~/Library/Application Support/WireDesk/client.log.YYYY-MM-DD`: `INFO IPC: post-cleanup drain: events_drained=N elapsed=...`.
...
**Вывод в обратную сторону (host → mac) UTF-8-безопасен с 2026-09-11.** До
этого буква, попавшая на границу пакета, приходила как два `�`: вывод резался
по `MAX_PAYLOAD`, и каждый кусок декодировался отдельно. Симптом выглядел как
«теряется по 2 байта в длинных русских строках», обходился через `--compress`
(base64 — ASCII, его резать безопасно) и подозревался в переполнении буфера
UART. Теперь незавершённый символ ждёт свой хвост
(`crates/wiredesk-exec-core/src/utf8_stream.rs`), а `--compress` для
сохранности текста больше не нужен — только для объёма. Разбор — в
`docs/briefs/wd-exec-utf8-byte-loss.md`.
