...
Release-сборка стартует фоновым tray-приложением — консольного окна не будет. В трее — глиф монитора, цвет кодирует состояние линка (серый до handshake, жёлтый/зелёный дальше).
...
🔴 **Проверять настроенный автозапуск надо не в диспетчере задач.** Вкладка «Автозагрузка» показывает только ключ `Run` и папку Startup, задачи планировщика в неё не попадают вообще — так что успешно настроенный автозапуск выглядит там как пустота. Смотреть:

```powershell
schtasks /Query /TN "WireDesk Host" /V /FO LIST
```

или `taskschd.msc` → Библиотека планировщика заданий → **WireDesk Host** (триггер `At log on of any user`, в свойствах — галка «Run with highest privileges»). Проверить, не дожидаясь логона: выделить задачу и нажать **Run**.

Обратное читается так: **запись видна в диспетчере задач и задачи при этом нет — значит сработал откат на `Run`**, элевированную задачу завести не удалось, и клики по окнам приложений, запущенных от администратора, работать не будут. Видны оба — значит заряжены оба, и это надо разобрать: либо не прошло удаление `Run` после создания задачи (в `host.log` тогда лежит `autostart: logon task created but Run value lingers`), либо ключ добавлен руками — стартовая сверка его не трогает, пока задача указывает куда надо. При двух заряженных механизмах хост на логоне стартует дважды, и кто из них выиграет у mutex’а единственного экземпляра — как повезёт; лишний `Run` снимается тем же чекбоксом (снять → Save → поставить → Save от администратора). Нет ни там, ни там — не удалось ни то, ни другое: окно Settings в этом случае пишет «Saved, but autostart toggle failed: …», а в `host.log` остаётся строка с `autostart:`.

Мелочь, которая пугает зря: `Last Run Time: 30-Nov-99 12:00:00 AM` у свежей задачи означает «ни разу не запускалась», а не ошибку.
