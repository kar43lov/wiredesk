...
**Справедливость канала.** `pump_shell_events` обходит слоты в порядке `[Pty, Exec]` (`pump_order`), и при открытом pty exec получает `PUMP_BUDGET_EXEC_SHARED = 4` чанка за такт вместо 16 (`exec_pump_budget`). Причина — `transport.send` блокирующий, и `tick` не зовёт `recv` пока шлёт: полный бюджет это 64 КБ подряд, то есть ~218 мс тишины на serial и ~533 мс на RFCOMM. Без открытой консоли бюджет прежний, так что одиночный `wd --exec` не потерял ни миллисекунды.
