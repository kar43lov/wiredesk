...
27 message types: HELLO/HELLO_ACK (handshake with screen resolution **и поколением протокола хоста** — см. ниже), 5 input types (mouse move/button/scroll, key down/up), 3 clipboard types (offer/chunk/ack) + ClipDecline (0x23), heartbeat/error/disconnect, 8 shell types (open/input/output/close/closed/exit + ShellOpenPty + PtyResize) и 5 pty-slot types (PtyOpen/PtyInput/PtyOutput/PtyClose/PtyExit, 0x48–0x4C).
