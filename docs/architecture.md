...
- **Adaptive heartbeat timeout (host)**. `Session::heartbeat_timeout()` возвращает 30s когда `clipboard.transfer_in_flight()` **или** `self.shell.is_some()` (PR #20), иначе 6s. CH340 bidirectional saturation топит heartbeats peer'а во время image transfer'а или heavy shell-output (ES `_search` ~24KB) и строгий 6s давал false-positive disconnect'ы. Pure helper `heartbeat_timeout_for(clipboard_busy, shell_open)` для unit-test'а без spawn'а ShellProcess'а.
- **IPC post-run drain (Mac client)**. После `run_oneshot` IPC handler шлёт ShellClose и **держит `single_inflight`** пока wire не стихнет: 2s idle deadline reset на каждом event'е, ShellExit short-circuit'ит, hard-cap 30s (PR #21). Без этого heavy-failed cmd оставлял wire saturated tail'ом ещё 30s — next `wd --exec` сел бы на dirty wire и тоже бы упал (cascade). Drain-marker в `client.log`: `INFO IPC: post-cleanup drain: events_drained=N elapsed=...`. Trade-off: +2s overhead на каждый Ok exit (можно сделать drain only on Err — easy follow-up).
