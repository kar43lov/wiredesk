...
**Runs alongside the GUI (macOS).** You no longer have to quit `WireDesk.app` to use `wd`. When the GUI is running it holds the serial port; `wd` connects to the GUI's Unix-socket relay and streams the PTY session through it — so you can poke the Host by hand while the GUI keeps an active capture/clipboard session going. The startup banner prints the source ("interactive via GUI IPC" vs "interactive via direct serial"). Ctrl+] exits cleanly either way. If the GUI is closed, `wd` opens the serial port directly, exactly as before.

**Interactive `wd` and `wd --exec` run at the same time** (since 2026-09-12). The Host keeps two shell slots — a pipe-mode one for `wd --exec` and a PTY one for the console — so an agent can keep running commands while you work in the terminal. Which protocol is in use is the Host's call: it announces its generation in `HelloAck`, and the client only uses the second slot when the Host has one. Against a Host built before this, the old rule still applies — whichever started first keeps the single slot and the other fails fast with "shell busy". The same refusal remains in every build for a *second* interactive `wd`: there is only ever one PTY slot. Exit codes differ by direction: a refused `wd --exec` exits 125 (transport class); a refused interactive `wd` exits 1 (the "shell busy" text still prints on stderr).

Both sides share one wire, so a `wd --exec` dumping hundreds of KB yields to the console while it streams (a quarter of its per-tick budget) — enough on the 3 Mbaud serial link, noticeable on Bluetooth.
