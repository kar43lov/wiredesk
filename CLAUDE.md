...
- **Тесты:** 279 client + 148 host + 94 exec-core + 88 protocol + 51 transport (BLE+factory+reconnect+detect) + 45 term + 18 wiredesk-core = 723 passing (+5 ignored). Host-side flaky на parallel runner'е macOS — `cargo test --workspace -- --test-threads=1`.
...
`docs/briefs/interactive-wd-via-gui-ipc.md` + `docs/plans/completed/20260703-interactive-wd-via-gui-ipc.md` — interactive `wd` через GUI IPC (**SHIPPED 2026-07-03**, 723 теста; последний direct-serial-путь устранён).
