...
- **Ввод:** мышь, клавиатура (включая кириллицу через scancodes), системные шорткаты через CGEventTap (Cmd+Space → Win+Space). Karabiner ⌥/⌘ swap toggle в Settings.
...
- **`wd --exec COMMAND`** (non-interactive mode для AI-агентов / scripted use) — single-shot через serial с UUID-tagged sentinel, `--ssh ALIAS` chain, `--timeout N`. На timeout печатает last 256 байт wire-buffer'а в stderr (`format_timeout_diagnostic`). Подробно: `docs/wd-exec-usage.md`.
...
- **Тесты:** 148 client + 97 host + 60 protocol + 45 term + 4 transport = 354. Host-side flaky на parallel runner'е macOS — `cargo test --workspace -- --test-threads=1`.
