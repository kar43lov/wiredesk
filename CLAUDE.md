...
- **Transport options:** USB-Serial null-modem (default, ~11 KB/s через CH340 @ 115200) или Bluetooth LE Central/Peripheral (`transport = "bluetooth"` в config.toml, ~30-100 KB/s через btleplug+WinRT GATT). Выбор через Mac Settings UI или TOML — Save+Restart pattern. AC0 verified live 2026-05-06: Continent-АП **не вмешивается** в BLE pathway. Custom GATT service с TX char (Notify Win→Mac) + RX char (WriteWithResponse Mac→Win), packet fragmentation через 4-byte ChunkHeader (240 bytes/chunk @ ATT MTU 247). Подробно: `docs/bluetooth-transport.md`.
