...
- **`wd --exec COMMAND`** (non-interactive mode для AI-агентов / scripted use) — single-shot с UUID-tagged sentinel, `--ssh ALIAS` chain, `--timeout N` (default 90s). На timeout печатает last 256 байт wire-buffer'а в stderr (`format_timeout_diagnostic`). Подробно: `docs/wd-exec-usage.md`.
...
- **Тесты:** 161 client + 97 host + 60 protocol + 21 term + 52 exec-core + 4 transport = 395. Host-side flaky на parallel runner'е macOS — `cargo test --workspace -- --test-threads=1`.
