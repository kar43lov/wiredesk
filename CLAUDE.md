...
- Провод один: пока консолью пользуются (ввод/вывод за последние 2 с), `wd --exec` получает четверть бюджета канала — 4.15 МБ за 18.4 с против 15.2 с соло; молчащая открытая консоль exec не тормозит (замер 14.09.2026). На RFCOMM/BLE `send` только ставит в очередь стека, и exec дозируется подтверждениями приёма от клиента (`RxProgress`, протокол v3) — эхо под выгрузкой 0.13 с (без обновления обеих сторон — 0.84 с). 🔴 Бюджет вывода хоста считается в байтах, не в чтениях: при счёте в чтениях построчный вывод упирался в частоту тактов, а не в провод (×10 медленнее)
...
`docs/plans/completed/20260912-dual-shell-slots.md` — два shell-слота на хосте (**SHIPPED в main 2026-09-12**, PR #30): интерактивный `wd` и `wd --exec` работают одновременно, взаимного «shell busy» между ними больше нет. Долг по AC2/AC3/AC6 закрыт 14.09.2026 (PR #31), AC8 на RFCOMM пройден в тот же день с подтверждениями приёма `RxProgress` — эхо под выгрузкой 0.84 → 0.13 с (замеры в конце плана).
