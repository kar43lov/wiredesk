...
- **`wiredesk-term`** CLI (отдельный бинарь). Raw-mode bridge для Ghostty/iTerm. Serial-port расщеплён через `Transport::try_clone()` на independent reader/writer handles — reader thread больше не держит mutex на blocking recv'е, stdin keystrokes доходят без latency. **Три потока**: reader (owns reader handle), main (stdin → cooked-mode buffer → writer.lock), heartbeat (`Heartbeat` каждые 2s через writer.lock). Все три держат общий `stop: AtomicBool`.
  ...
  - **`--exec` non-interactive mode** (`run_oneshot` рядом с `bridge_loop`). Опциональный `--ssh ALIAS` для chain'а через `ssh -tt`. Не enable raw mode, не открывает stdin. Pure helpers `format_command(uuid, ShellKind, cmd)` (PS-вариант **обязательно** wrap'ит cmd в `try {…} catch { $LASTEXITCODE = 1 }` — без этого terminating error не emit'ит sentinel и process висит до timeout 124), `parse_sentinel(line, uuid) -> Option<i32>` (anchored prefix-strip + `parse::<i32>` отсеивает stdin-echo с literal `$?`/`$LASTEXITCODE`), `clean_stdout(buf, uuid)` (срезает всё до последнего prompt'а до sentinel'а, сам sentinel, и filter'ует строки содержащие unexpanded sentinel — это echo от ssh -tt; PS pipe-mode без PSReadLine не echo'ит → echo-line отсутствует и filter no-op). State machine `OneShotState::{AwaitingHostPrompt, AwaitingRemotePrompt, AwaitingSentinel}` гоняется через line-walker над накопительным `pending` буфером, отдельный `full_log` хранит full conversation для финального `clean_stdout`. Heartbeat thread активен — host's idle timeout не разорвёт connection во время slow-running команды. На timeout `--timeout SECONDS` (default 30) — exit 124 как `timeout(1)`. Persistent SSH делается через OpenSSH ControlMaster в `~/.ssh/config` host'а — не наш код.
