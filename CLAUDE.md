...
- **Transport options:** USB-Serial null-modem (default, ~11 KB/s через CH340 @ 115200) или Bluetooth LE (`transport = "bluetooth"` в config.toml). **BLE shipped 2026-05-06, but live-measured ~4-5 KB/s — *slower* than serial** на Mac M4 + Win11 reference setup. Continent-АП verified не вмешивается в BLE pathway (AC0 ✓), но реальный throughput не дотянул до brief'овой оценки 30-100 KB/s — bottleneck в btleplug/WinRT GATT layer. **Default остаётся serial**; BLE — fallback "no-cable mode" с честным perf disclaimer. Реальный speedup — Plan A (FT232H @ 3 Mbaud, ~300 KB/s, ждёт hardware purchase). Подробно: `docs/bluetooth-transport.md`.
...
- **Тесты:** 172 host + 104 client + 84 protocol + 60 exec-core + 41 transport (BLE+factory+reconnect) + 22 term + 4 wiredesk-core = 487 passing (+3 ignored = 490 total). Host-side flaky на parallel runner'е macOS — `cargo test --workspace -- --test-threads=1`.
