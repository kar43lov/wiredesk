...
- **Settings window** (через tray): port (TextEdit), baud, width/height, чекбокс «Run on startup», кнопки Copy Mac launch command / Save / Hide. Save пишет TOML и в случае autostart toggles HKCU registry; sysmessage «Saved. Restart WireDesk Host to apply.» (Save+Restart pattern).
...
- **Settings panel** в chrome-UI (collapsing «Settings»): port (combo + free-text), baud, host screen W×H, client name. Save пишет `~/Library/Application Support/WireDesk/config.toml` и показывает inline toast 3 секунды. В capture/fullscreen settings panel скрыта (info-only screen без интерактивных элементов).
...
ui/
  ...
  settings_window.rs   — #[cfg(windows)] nwg builder UI, read_form(), set_status()
  tray.rs              — #[cfg(windows)] TrayUi with embedded PNGs, popup menu
  ...
  single_instance.rs   — CreateMutexW("WireDeskHostSingleton") + drop=close
                          (own implementation, no single-instance crate)
